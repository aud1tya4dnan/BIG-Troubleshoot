...
ip_big4 = os.environ.get('IP_BIG_4')
ip_big5 = os.environ.get('IP_BIG_5')
...
hosts = [
    ...
    {"ip": ip_big4, "name": "big4"},
    {"ip": ip_big5, "name": "big5"},
]
...
test_scripts = {
    "io": "/home/BIG-Troubleshoot/SYSBENCH-script/sysbench_io_test.sh",
    "mem": "/home/BIG-Troubleshoot/SYSBENCH-script/sysbench_mem_test.sh",
    "cpu": "/home/BIG-Troubleshoot/SYSBENCH-script/sysbench_cpu_test.sh",
}
...
def run_test_on_host(host, vm_count, test_type, script_path):
    try:
        s = pxssh.pxssh()
        s.login(host["ip"], "root", PASSWORD)
        print(f"[{host['name']}] Connected.")
...
def main():
    ...
    try:
        vm_count = int(input("Mau dijalankan di berapa VM? (1 / 2 / 3 / 4 / 5): ").strip())
        if vm_count not in [1, 2, 3, 4, 5]:
            raise ValueError
    except ValueError:
        print("❌ Jumlah VM tidak valid.")
        return
    ...
    threads = []
    for host in selected_hosts:
        t = threading.Thread(target=run_test_on_host, args=(host, vm_count, test_type, script_path))
        t.start()
        threads.append(t)
