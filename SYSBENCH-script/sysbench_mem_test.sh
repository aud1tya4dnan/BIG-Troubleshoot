...
echo "Iteration,Operation,Block Size,Total Size,Operations/sec,Transferred (MB/sec - converted from MiB/sec)" > "$CSV_READ"
...
for i in $(seq 1 $ITERATIONS); do
    ...
    # Extract MiB/s, then convert to MB/s
    mib_sec=$(echo "$OUTPUT" | grep "transferred" | awk '{print $4}' | tr -d '()')
    mb_sec=$(awk "BEGIN {printf \"%.2f\", $mib_sec * 1.048576}")
...
echo "Iteration,Operation,Block Size,Total Size,Operations/sec,Transferred (MB/sec - converted from MiB/sec)" > "$CSV_WRITE"
